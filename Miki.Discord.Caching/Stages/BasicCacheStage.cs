...
namespace Miki.Discord.Caching.Stages
{
	public class BasicCacheStage : ICacheStage
	{
		...
		private async Task OnReady(GatewayReadyPacket ready, IExtendedCacheClient cache)
		{
			...
			await cache.HashUpsertAsync(CacheUtils.UsersCacheKey, "me", ready.CurrentUser);
			await cache.HashUpsertAsync(CacheUtils.UsersCacheKey, ready.CurrentUser.Id.ToString(), ready.CurrentUser);
		}
