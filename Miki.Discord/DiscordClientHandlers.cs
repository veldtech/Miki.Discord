...
namespace Miki.Discord
{
    public partial class DiscordClient
    {
        ...
        private async Task OnGuildEmojiUpdate(ulong guildId, DiscordEmoji[] emojis)
        {
            ...
            {
                guild.Emojis = emojis;
                await CacheClient.HashUpsertAsync(CacheUtils.GuildsCacheKey, guildId.ToString(), guild);
            }
        }
        ...
        private async Task OnUserUpdate(DiscordPresencePacket user)
        {
            ...
            {
                CacheClient.HashUpsertAsync(CacheUtils.UsersCacheKey, user.User.Id.ToString(), user.User)
            };

            if (user.GuildId.HasValue)
            {
                var guildMember = await CacheClient.HashGetAsync<DiscordGuildMemberPacket>(CacheUtils.GuildMembersKey(user.GuildId.Value), user.User.Id.ToString());
                if (guildMember == null)
                {
                    guildMember = new DiscordGuildMemberPacket();
                }
                guildMember.User = user.User;
                tasks.Add(CacheClient.HashUpsertAsync(CacheUtils.GuildMembersKey(user.GuildId.Value), user.User.Id.ToString(), guildMember));
            }
        ...
        {
            var m = await CacheClient.HashGetAsync<DiscordGuildMemberPacket>(CacheUtils.GuildMembersKey(member.GuildId), member.User.Id.ToString());
        ...
        {
            DiscordGuildPacket guild = await CacheClient.HashGetAsync<DiscordGuildPacket>(CacheUtils.GuildsCacheKey, arg1.Id.ToString());
        ...
        private Task OnGuildCreate(DiscordGuildPacket guild)
        {
            ...
            return Task.WhenAll(
                ...
                CacheClient.HashUpsertAsync(CacheUtils.GuildMembersKey(guild.Id), guild.Members.Select(x =>
                {
                    x.GuildId = guild.Id;
                    return new KeyValuePair<string, DiscordGuildMemberPacket>(x.User.Id.ToString(), x);
                })),
                CacheClient.HashUpsertAsync(CacheUtils.GuildRolesKey(guild.Id), guild.Roles.Select(x =>
                {
                    return new KeyValuePair<string, DiscordRolePacket>(x.Id.ToString(), x);
                })),
                CacheClient.HashUpsertAsync(CacheUtils.UsersCacheKey, guild.Members.Select(x =>
                {
